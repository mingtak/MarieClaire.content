...
class CalculateWeight(ManaBasic):
    ...
    def __call__(self):
        ...
        csv_file = request.get('csv_file').split(',')[1]
...
class UpdateCalculateWeight(ManaBasic):
    def __call__(self):
        request = self.request
        try:
            line_item_id = json.loads(request.get('line_item_id'))
            if len(line_item_id) == 1:
                line_item_id.append('zzzz')
	    line_item_id = map(lambda x:str(x), line_item_id)

...
class CalculateWeight(ManaBasic):
    template = ViewPageTemplateFile('template/result_calculate_weight.pt')
    def __call__(self):
        request = self.request
	...
	for item in result:
        ...
        if not data:
	    return 'empty'
        # 初始min_imp 為int 最大值，再找出最小值
        min_imp = sys.maxint
        for k,v in data.items():
            if v[0] < min_imp:
                min_imp = v[0]
                min_clicks = v[1]
        est_allpass_imp_weight = round(float(est_imp) / float(min_imp), 1)
        est_allpass_tmp_imp = round(float(min_imp) * float(est_allpass_imp_weight))
        est_allpass_clicks_weight = round( float(est_ctr) / 100 * float(est_allpass_tmp_imp) / float(min_clicks), 1)

        allpass_data = {}
        for k,v in data.items():
            new_imp = round(float(v[0]) * est_allpass_imp_weight)
            new_clicks = round(float(v[1]) * est_allpass_clicks_weight)
            new_ctr = round(new_clicks / new_imp * 100, 1)
            allpass_data[k] = [ new_imp, new_clicks, new_ctr]

        # 將data排序
        tmp_mid_data = sorted(data.items(), key=lambda x:x[1][0])
        ...
        if len(tmp_mid_data) > 1:
            ...
            mid_index = 0
        # 抓中位數的值
        median_imp = tmp_mid_data[int(mid_index)][1][0]
        median_clicks = tmp_mid_data[int(mid_index)][1][1]
        mid_data = {}

        est_mid_imp_weight = round(float(est_imp) / float(median_imp), 1)
        est_mid_clicks_weight = round( float(est_ctr) /100 * median_imp * est_mid_imp_weight / median_clicks , 1)
        for k,v in tmp_mid_data:
            new_mid_imp = round(float(v[0]) * est_mid_imp_weight)
            new_mid_clicks = round(float(v[1]) * est_mid_clicks_weight)
 	    new_mid_ctr = round(new_mid_clicks / new_mid_imp * 100, 1)
            mid_data[k] = [ new_mid_imp, new_mid_clicks, new_mid_ctr]

        self.item_id_str = item_id_str
        self.est_allpass_imp_weight = est_allpass_imp_weight
        self.est_allpass_clicks_weight = est_allpass_clicks_weight
        self.est_mid_imp_weight = est_mid_imp_weight
        self.est_mid_clicks_weight = est_mid_clicks_weight
        self.data = sorted(data.items(), key=lambda x:x[1][0])
...
class ShowCalculateWeight(BrowserView):
    ...
    def __call__(self):
        ...
        if 'Manager' not in  api.user.get_current().getRoles():
            self.request.response.redirect(abs_url)
            api.portal.show_message(message='您無權限!', request=self.request, type='error')
        return self.template()



class UpdateCalculateWeight(ManaBasic):
    def __call__(self):
        request = self.request
        try:
            line_item_id = request.get('line_item_id').split(',')
            weight = json.loads(request.get('weight'))
	    action = request.get('action')
            start_date = request.get('start_date')
            end_date = request.get('end_date')
            execStr = """UPDATE dfp_ad_server SET im_weight={}, cli_weight={}, status='disabled' WHERE DATE BETWEEN 
               '{}' AND '{}' AND LINE_ITEM_ID in {}
               """.format(weight[0], weight[1], start_date, end_date, tuple(line_item_id))
            self.execSql(execStr)
            return 'success'
